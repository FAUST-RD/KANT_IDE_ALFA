...
Before an agent starts, `workspace.py` snapshots the project. When it finishes, `ClaudePane.show_ai_review()` inserts an `_AiReviewCard` directly into the chat transcript (same insert-into-`chat_layout` pattern as permission cards) rather than opening a separate modal dialog — a compact assistant card that "Controllo" expands into file/hunk choices and an editable final result, right there in the conversation. Checked hunks are rebuilt deterministically from `difflib` opcodes; unchecked hunks come from the snapshot, manual final text overrides the selected file, and cancel restores the entire snapshot. The card emits `resolved('apply'|'cancel')` once and locks; `workspace.py._finish_ai_review()` does the actual apply/rollback in that callback and reports the outcome as a follow-up chat message.
