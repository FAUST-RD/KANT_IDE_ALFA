...
Tag, name, nesting, `OPEN`/`CLOSED` placement, and `#stable-id` are never left for an AI to guess at — they are facts about the code, extracted deterministically. `.py` files get exact results from Python's own `ast` module. Go, TypeScript/JavaScript, C#, Java, and C++ each first try the language's own real toolchain — `go/ast`, an already-installed `typescript` package, Roslyn via the .NET SDK's own bundled compiler assemblies, the JDK's Compiler Tree API, `clang -ast-dump=json` — falling back to a tested heuristic scanner only when that toolchain isn't installed. Nothing here ever reaches the network: every toolchain path uses only what's already on disk (no package install, no NuGet/npm fetch). An AI is only ever asked to write the two description lines a marker leaves blank, never the structure around them.
